-- LLR-BNK-01.1: Bank Statement Upload — stores uploaded bank statement metadata
CREATE TABLE IF NOT EXISTS af_novadesk.bnk_statements (
    ...
    file_type                   VARCHAR(10)  NOT NULL,            -- CSV, XLSX, XLS, PDF
    file_size_bytes             INTEGER      NOT NULL,
    is_encrypted                BOOLEAN      NOT NULL DEFAULT false,
    file_password_hash          VARCHAR(255),                     -- SHA-256 hash of the password (to detect re-upload)
...
CREATE INDEX IF NOT EXISTS idx_bnk_statements_status ON af_novadesk.bnk_statements (statement_status);

COMMENT ON TABLE af_novadesk.bnk_statements IS 'Uploaded bank statements for reconciliation — LLR-BNK-01.1';
...
COMMENT ON COLUMN af_novadesk.bnk_statements.file_password_hash IS 'SHA-256 hash of the provided file password; used to detect re-upload of same encrypted file';
