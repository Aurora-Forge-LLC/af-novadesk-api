-- LLR-BNK-01.4: Create bnk_transactions table for extracted bank statement transactions
CREATE TABLE IF NOT EXISTS af_novadesk.bnk_transactions (
    ...
    matched_expense_id    UUID,                             -- FK to exp_expense_transactions (added later)
    ...
    CONSTRAINT chk_bnk_txn_status CHECK (status IN ('ACTIVE','INACTIVE','SUSPENDED','DELETED')),
    CONSTRAINT chk_bnk_txn_reconciliation_status CHECK (reconciliation_status IN ('UNMATCHED','SUGGESTED','MATCHED','IGNORED')),
    CONSTRAINT chk_bnk_txn_matching_method CHECK (matching_method IS NULL OR matching_method IN ('AUTO_MATCHED','USER_CONFIRMED','MANUAL'))
);
