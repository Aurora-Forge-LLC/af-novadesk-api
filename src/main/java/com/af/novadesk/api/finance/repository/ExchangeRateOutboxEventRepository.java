...
/**
 * JPA repository for {@link ExchangeRateOutboxEvent}.
 *
 ...
 * @Transactional
 * public void poll() {
 *     List<ExchangeRateOutboxEvent> batch =
 *         repository.findPendingEvents(Instant.now(), Pageable.ofSize(100));
 *     for (ExchangeRateOutboxEvent event : batch) {
 *         try {
 ...
 */
@Repository
public interface ExchangeRateOutboxEventRepository
        extends JpaRepository<ExchangeRateOutboxEvent, UUID> {
     ...
     */
    @Query("""
            SELECT e FROM ExchangeRateOutboxEvent e
    ...
    List<ExchangeRateOutboxEvent> findPendingEvents(
