...
/**
 * Transactional Outbox for the {@link ExchangeRate} aggregate (LLR-FIN-02.3).
 *
 * <p>Every successful or failed exchange-rate sync attempt produces exactly one
 * outbox event <em>in the same database transaction</em> as the rate persistence.
 * The polling publisher reads {@code PENDING} rows and delivers them to the
 * message broker after commit, guaranteeing at-least-once delivery without
 * distributed transactions.</p>
 *
 * <h2>Polling query</h2>
 * <pre>{@code
 * SELECT * FROM af_novadesk_outbox.exchange_rate_outbox_events
...
@Entity
@Table(
        name = "exchange_rate_outbox_events",
        schema = "af_novadesk_outbox",
        indexes = {
                // Poller's primary scan — ordered by arrival time
                @Index(columnList = "status, created_at",  name = "idx_er_outbox_status_created"),
                // Look up all events for a specific ExchangeRate
                @Index(columnList = "aggregate_id",        name = "idx_er_outbox_aggregate_id"),
                // Multi-tenant / entity-scoped debugging
                @Index(columnList = "organization_id",     name = "idx_er_outbox_org_id"),
                // Efficient retry polling — partial index on retryable states
                @Index(columnList = "status, next_retry_at", name = "idx_er_outbox_next_retry")
        },
        uniqueConstraints = {
                @UniqueConstraint(columnNames = "idempotency_key", name = "uk_er_outbox_idempotency_key")
        }
)
...
@ToString(exclude = {"exchangeRate"})
public class ExchangeRateOutboxEvent extends AbstractEntity {
    ...
    /**
     * The {@link ExchangeRate} record whose creation produced this event.
     * FK ensures referential integrity; LAZY fetch since the poller only needs
     * the payload, not the full aggregate graph.
     * <p>Nullable — failure events may not have a persisted rate.</p>
     */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "aggregate_id",
            foreignKey = @ForeignKey(name = "fk_er_outbox_exchange_rate"))
    ...
    private ExchangeRateEventType eventType;
     ...
     *
     * <p>Minimum payload structure for {@code EXCHANGE_RATE_SYNC_COMPLETED}:</p>
     * <pre>{@code
     * {
     *   "rateId": "<uuid>",
     *   "sourceCurrency": "INR",
     ...
     * }</pre>
     *
     * <p>Payload structure for {@code EXCHANGE_RATE_SYNC_FAILED}:</p>
     * <pre>{@code
     * {
     *   "sourceCurrency": "INR",
     *   "targetCurrency": "USD",
     ...
     * Null for scheduler-initiated events (system).
     */
     ...
     * Convention: {@code "<ExchangeRateEventType>:<rateDate>:<sourceCurrency>-><targetCurrency>"}.
